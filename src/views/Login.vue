  ...
  setTimeout(() => {
    invoke<AuthenticationResult>('authenticate', {
      ...
      .then((auth) => {
        ...
        participant.id = parseInt(sessionStorage.getItem('PARTICIPANT_ID') as string)
        loading.finish();
        setTimeout(() => {
    ...
    <NCard style="width: 500px; padding: 2rem">
      ...
      <NForm ref="form" :model="model" :rules="rules">
        <NFormItem path="username" label="Username">
          <NInput ref="inputUsername" v-model:value="model.username" @keydown="onEnter" size="large" />
        </NFormItem>
        <NFormItem path="password" label="Password">
          <NInput type="password" ref="inputPassword" v-model:value="model.password" @keydown="onEnter" size="large" />
        </NFormItem>
      </NForm>
      <NSpace vertical align="center" style="margin-top: 1rem">
        <NText italic @click="$router.replace('/signup')" style="cursor: pointer">No account? Sign up here!</NText>
        <NButton ref="authButton" primary type="primary" style="margin-top: 1rem" @click="handleAuthentication">Log In
        </NButton>
      </NSpace>
    </NCard>
  </NSpace>
