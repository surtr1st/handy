...
import { AuthenticationResult, SnakeUser } from '../types'
import {
  NCard,
  NForm,
  NFormItem,
  ...
  setTimeout(() => {
    invoke<AuthenticationResult>('authenticate', {
      ...
      .then((auth) => {
        ...
        const userId = auth[1]
        onSuccess(message);
        useLocalStorage('USER_ID', userId)
        loading.finish()
        setTimeout(() => {
          replace('/mainpage/getting-started')
        }, 500);
      })
      .catch((message) => { loading.error(); onError(message as string) });
  }, 300)
}
    ...
    <NCard style="width: 500px; padding: 2rem">
      ...
      <NForm ref="form" :model="model" :rules="rules">
        ...
        <NFormItem path="password" label="Password">
          <NInput type="password" v-model:value="model.password" size="large" />
        </NFormItem>
      </NForm>
      <NSpace vertical align="center" style="margin-top: 1rem">
        ...
        <NButton primary type="primary" style="margin-top: 1rem" @click="handleAuthentication">Log In</NButton>
      </NSpace>
    </NCard>
  </NSpace>
</template>
