...
<template>
  <RouterLink :to="`/mainpage/iterations/${iid}/backlogs/${props.id}`" @click="handleBacklogChosen(props.id)"
    style="text-decoration: none">
    <NCard :title="props.title" hoverable style="margin-bottom: 0.7rem">
      <template #header-extra>
        <NBadge :value="statusLabel" :type="statusColor ?? 'error'" style="margin-right: 0.5rem" />
      ...
      <template #action>
        <NSpace justify="space-evenly">
          <NStatistic label="Tasks" value="0">
            <template #suffix>/ 0</template>
          </NStatistic>
          <NStatistic label="Hours" value="0">
            <template #suffix>/ {{ props.hours }}</template>
          </NStatistic>
          <NStatistic label="Points">
            <template #prefix>
              <NProgress type="circle" :color="colors.primary" :percentage="
          ...
          <NStatistic label="Status">
            <NPopselect v-model:value="option" :options="progressOptions"
