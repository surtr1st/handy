...
<template>
  ...
  <NGrid v-else cols="3">
    <NGi span="1" v-for="iteration in iterations">
      <NCard :title="iteration.title" hoverable style="width: 430px">
        <NThing content-style="font-size: 18px">
          <NGrid :cols="4">
            <NGi :span="2">
              <NSpace vertical>
                <NStatistic label="Id" :value="`#${iteration.id}`" />

                <NStatistic label="Created By" :value="iteration.created_by" />
                <NStatistic label="Start Date" :value="useFormattedDate(iteration?.start_date)" />
                <NStatistic label="End Date" :value="useFormattedDate(iteration?.end_date)" />
                <NButton primary type="primary" style="width: 150px" @click="debounceJoin(iteration.id)">
                  Join
                </NButton>
              </NSpace>
            </NGi>
